...
export class AuthService {
  ...
  async registerUser(user: User, resp: Response, birthday: Date) {
    const { name, email, password } = user;

    // Check for required fields
    if (!name?.trim() || !email?.trim() || !password.trim() || !birthday) {
      return resp.status(400).send({ message: 'Not all required fields have been provided.' });
    }
    ...
    try {
      const newUser = await this.userRepository.save({
        ...
        birthday, // This should be the date parameter
      });

      console.log(newUser);

      return resp.status(200).send(newUser);
    } catch (error) {
      console.error(error);
      ...
      return resp.status(500).send({ message: 'An error occurred during registration.' });
    }
